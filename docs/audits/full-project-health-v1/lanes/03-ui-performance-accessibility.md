...
- Existing targeted component tests passed, but the baseline Playwright axe helper was a false-confidence wrapper because it did not assert returned violations. Repair wave later tightened this helper; keep this lane section as baseline evidence, not current state.
...
| E-L03-006 | file | `tests/e2e/helpers/axe.ts:31-37`; copied to `docs/audits/full-project-health-v1/evidence/03-ui-performance-accessibility/e2e-axe-helper.txt` | Baseline `checkA11y` called `builder.analyze()` but did not inspect/assert `violations`; it also ignored `_context` and `_options`. | This proved the baseline helper was not a failing a11y gate; repair wave later tightened it. |
| E-L03-007 | file | `tests/e2e/homepage.spec.ts:314-317`; copied to `docs/audits/full-project-health-v1/evidence/03-ui-performance-accessibility/homepage-axe-usage.txt` | Homepage test calls `checkA11y`. | With the baseline helper, this usage could pass despite violations; repair wave later changed helper behavior. |
