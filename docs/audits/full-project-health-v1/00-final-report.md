...
结论先说：**基线审计时本地技术基线是绿的，但 Cloudflare preview 的联系页是红的；后续 repair wave 已经把 preview contact 500 修到 deployed smoke 通过。当前仍不能把它当作“正式公开上线已准备好”。**
...
- **基线预览没有完整跑通**：Cloudflare preview gateway 上 `/en/contact` 和 `/zh/contact` 返回 `500`，web worker tail 明确报 Next Cache Components `blocking-route`。
- **repair wave 已关闭 preview contact blocker**：后续 deployed smoke 对 preview gateway 返回 `All checks passed`，`/en/contact` 与 `/zh/contact` 当前返回 `200`。见 `07-repair-closure.md`。
- **公开上线还不稳**：买家能看到的信任材料还有硬缺口：占位手机号、产品占位图和真实外部 lead delivery 凭证仍未补齐。ISO PDF、AS/NZS 标准说法、产品询盘测试假绿等已在 repair wave 中处理。
...
Cloudflare preview deployed smoke: Pass after repair wave
Public launch readiness: No-go until parked owner-content and external lead-delivery credentials are resolved or explicitly removed from launch scope
Production/deployed truth: Preview gateway proved for route smoke after repair; production domain still not proved
```

本轮没有确认 P0。基线审计产生 6 个 P1；repair wave 已关闭其中的 runtime/test/content-proof 类问题，`FPH-001` 与 `FPH-003` 继续 parked，等待 owner 提供真实公开电话和产品素材。
...
基线 runtime proof 曾经证明：

- preview phase6 三个 worker 可以部署成功。
- preview gateway URL 已脱敏记录。
...
repair wave 之后的 fresh proof：

- `pnpm smoke:cf:deploy -- --base-url <redacted-workers-dev-gateway-url>` 返回 `All checks passed`。
- `/en/contact` 和 `/zh/contact` 当前返回 `200`。
...
另外，preview 现在只能说“route smoke 能跑”，不能说“完整 buyer flow 能跑”。当前 preview secret proof 仍不能证明 Resend / Airtable / Turnstile 外部投递链，所以真实 lead delivery 不能宣称通过。
...
| ID | 问题 | 影响 | Linus Gate |
| --- | --- | --- | --- |
| FPH-000 | 基线 Cloudflare preview `/en/contact` 和 `/zh/contact` 返回 500 | 主询盘入口不可用；repair wave 后 route smoke 已通过 | Keep |
| FPH-001 | 联系手机号还是 `+86-518-0000-0000` | 买家信任受损；等待 owner 提供真实号码 | Needs proof |
| FPH-002 | ISO 证书配置有 `/certs/iso9001.pdf`，但 public 文件不存在 | 证书可信度缺口；repair wave 已移除缺失链接 | Keep |
| FPH-003 | 多个 live 产品族还在用 `sample-product.svg` | 产品展示像未完成；等待真实素材 | Needs proof |
| FPH-004 | AS/NZS 标准说法在公开文案和产品数据中冲突 | 合规表达不可信；repair wave 已收口 | Keep |
| FPH-005 | 产品询盘测试声称真实校验，但 mock 了关键 schema | 关键询盘保护链测试假绿；repair wave 已改用真实 schema | Keep |

这些 P1 是基线审计发现。repair wave 后，仍不能靠代码自行关闭的是 `FPH-001` 和 `FPH-003`，因为它们依赖 owner 的真实电话和产品图片素材。
...
- 基线发现 Playwright axe helper 只调用 `analyze()`、contact smoke 里有永真 ARIA 断言；repair wave 已把 helper 改成会 assert violations，并保留 selector-scoped checks。
...
但公开内容、买家信任材料和转化路径仍有上线缺口：

- 电话号像占位。
- 多个产品还显示 sample placeholder。
- 产品页的询盘 CTA 仍是泛 `/contact`，没有保留产品上下文。

基线发现的 ISO PDF 缺失链接和 AS/NZS 2053 / 61386 表达冲突已在 repair wave 中收口；不要再把它们当作当前 confirmed gap。

Google 侧事实全部 blocked：没有 Search Console、URL Inspection、CrUX、PageSpeed 数据。

## 8. Test Value and AI-Smell Findings

测试数量很多，基础命令也绿。基线审计发现过几类“看起来有证明，实际证明不足”的问题：

- Knip 配置把 `src/components/**` 和 `src/lib/**` 设得太宽，dead-code 检测容易假绿。
- UI primitive tests 数量偏膨胀，和 buyer-flow proof 的优先级不匹配。

repair wave 已处理 `/api/inquiry` schema mock、axe helper assertion、contact smoke 永真断言、以及 mutation guard 缺失脚本问题。剩余测试质量风险主要是 broad Knip entries、低价值 primitive test 膨胀，以及全量 mutation score 未运行。
...
| Claim | Status | Why |
| --- | --- | --- |
| Cloudflare preview deploy/auth truth | Proved for route smoke | preview deploy passed with redacted env-file proof |
| Preview contact page runtime | Fixed after repair wave | `/en/contact` and `/zh/contact` now return 200 in deployed smoke |
