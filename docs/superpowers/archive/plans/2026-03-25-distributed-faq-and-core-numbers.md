...
**Codex Review:** Round 1 scored 5/10, NEEDS_REVISION. 8 of 13 issues accepted and fixed in this revision. See `docs/superpowers/archive/reviews/2026-03-25-distributed-faq-and-core-numbers-review.md`.

**BDD Specs Reference:** `docs/specs/batch3-faq-numbers/bdd-specs.md`

**Design Handoff:** `docs/impeccable/faq/HANDOFF.md`
...
**IMPORTANT:** Do NOT remove the existing keys (pageTitle, categories, noResults, searchPlaceholder, contactCta) yet — the old `/faq` route still reads them. Add the new `sectionTitle` and `items` keys alongside the old ones. The old keys will be removed in Task 7 when the route is deleted. Extract all 20 Q&A items from `docs/cwf/faq/v1-final.md`.
...
All answer text comes from `docs/cwf/faq/v1-final.md`. Use `\n\n` for paragraph breaks within answers.

- [ ] **Step 2: Add same structure to `messages/zh/deferred.json`**

Extract Chinese content from `docs/cwf/faq/v1-zh-final.md`. Same keys, Chinese values.
