...
`src/lib/content/products-source.ts` and `products.ts` (MDX-based product system) are **not used** by the new product pages. Content is archived in `content/_archive/products/`. If they remain absent from the active tree, treat that as the intended cleanup outcome rather than as missing runtime functionality.
