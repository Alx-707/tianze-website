...
> - 如果页面改动涉及品牌、联系方式、默认 SEO、导航、页脚这类站点身份信息，先看 `src/sites/**`，不要在页面里继续写死。
> - 如果页面改动涉及市场结构、产品目录、market landing 的默认标题或标准体系，先看 `src/sites/**` 和 `src/constants/product-catalog.ts`，不要把这些真相继续散在页面里。
