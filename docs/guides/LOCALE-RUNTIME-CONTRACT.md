...
- Per-site overlays are a future-only extension and must not be treated as active runtime structure today.
- Site-specific facts belong in `src/config/single-site*.ts`; only user-facing prose that is part of the current single-site baseline belongs in `messages/**`.
