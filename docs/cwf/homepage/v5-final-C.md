# Tianze Homepage V5-C — Design Input

> **Status:** 设计输入文档（最小约束版）
> **Based on:** V5-final 文案 + V5-B 讨论结论
> **Date:** 2026-02-06
...
**一句话：** 自研弯管设备的管件制造商 — 从机器到管道，全链路技术自主。

设计应传递：工程精度、垂直整合的技术深度。造设备的人在造管道，这是核心差异化。

## 设计约束

### 唯一的硬约束

1. **内容元素** — 各 Section 定义了「需要呈现什么」（文案、图片、图标、数据），不规定「怎么摆」
2. **SVG 线条图标** — 不使用 emoji
3. **布局骨架** — 采纳 Vercel 布局规范（见下表）

### 布局骨架

**来源**: `docs/impeccable/external/vercel-design-system/`
...
配色、字体、圆角、阴影、背景节奏、动效风格、区块间分隔方式 — 全部由设计师决定。
...
| S3 产品线 | 产品广度 + 定制能力（Custom angles, OD, 包装） |
| S4 样品CTA | 低门槛承诺（免费样品, 3天发货） |
| S5 场景 | 应用想象 + 社会证明（6 场景 + 证言） |
| S6 服务承诺 | 服务交付方式（48小时图纸, 批次追溯, 出口包装） |
