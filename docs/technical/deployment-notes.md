...
### 真正执行 cleanup 的步骤（独立 PR）

前置条件：
- 当前 PR 已 merged 且 phase6 拓扑在生产稳定运行 ≥7 天
- 拥有可读写 `tianze-pipe.com` zone 的 Cloudflare token 或 dashboard 访问权
- 旧 `tianze-website*` 服务确认不再接流量

步骤：

1. **审计**
   - `wrangler list`（或 Dashboard → Workers）列出所有服务
   - 识别旧服务：`tianze-website` / `tianze-website-preview` / `tianze-website-production`
   - 用 `wrangler tail <name>` 确认每个旧服务在 24h 内无请求

2. **准备一次性 cleanup Worker**
   - 新建目录 `scripts/cloudflare/legacy-do-cleanup/`
   - `wrangler.cleanup.jsonc`：`name` 设为旧服务名（每个旧服务一份），不绑 R2/D1，仅保留 `migrations` 字段
   - `noop-worker.mjs`：空 fetch handler 返回 `410 Gone`
   - migration 关键字段：
     ```jsonc
     "migrations": [
       { "tag": "v1", "new_sqlite_classes": ["DOQueueHandler"] },
       { "tag": "v2-cleanup", "deleted_classes": ["DOQueueHandler", "DOShardedTagCache", "BucketCachePurge"] }
     ]
     ```
   - `tag` 必须严格在历史 `v1` 之后；如旧服务历史只有 `v1` 则用 `v2-cleanup`

3. **部署 cleanup Worker**
   - 对每个旧服务名分别跑：
     `wrangler deploy --config scripts/cloudflare/legacy-do-cleanup/wrangler.cleanup.<env>.jsonc`
   - 关键约束：必须用**旧服务名**重新部署，不能新建别名——DO migration 只对同名 Worker 生效

4. **验证 DO storage 已清空**
   - Dashboard → Workers → Durable Objects → 确认对应 namespace 计数归零
   - `wrangler tail <old-service>` 几分钟内看不到 DO 相关请求或错误
   - 必要时用 Cloudflare API `GET /accounts/{id}/workers/durable_objects/namespaces` 复核

5. **彻底删除旧服务**
   - `wrangler delete --name tianze-website-preview` 等
   - **不要**跳过步骤 3-4 直接 `wrangler delete`——没有 `deleted_classes` migration 就硬删，DO storage 会残留为 orphan

执行完后：
- 移除 `scripts/cloudflare/legacy-do-cleanup/` 目录（一次性使用，不留代码）
- 在本文档记录 cleanup 完成日期和受影响服务名
