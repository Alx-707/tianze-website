...
> 当前仓库额外规则
> - 当前先按“高标准单站 + 多站基础准备”来做，不要直接把页面需求变成多站结构施工。
> - 如果页面改动涉及品牌、联系方式、默认 SEO、导航、页脚这类站点身份信息，先看 `src/config/single-site*.ts`，不要在页面里继续写死。
> - 如果页面改动涉及市场结构、产品目录、market landing 的默认标题或标准体系，先看 `src/config/single-site-product-catalog.ts` 和 `src/constants/product-catalog.ts`，不要把这些真相继续散在页面里。
> - 如果页面改动涉及文案、MDX、消息、SEO 默认值，先判断它属于站点身份层、内容资产层，还是页面表现层。
...
**推荐交付物**：
- `docs/cwf/context/company/` 下的一组已定稿业务资料（由你定稿）
...
**推荐最小闭环**：
- 你提供资料 → LLM 整理并补全 `docs/cwf/context/company/` 下的事实资料 → 你审核定稿 → 再进入 Phase -1/0/1/… 输出页面
