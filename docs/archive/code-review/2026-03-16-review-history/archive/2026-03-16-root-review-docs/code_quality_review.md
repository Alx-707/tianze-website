...
- ✅ TypeScript/ESLint/Test/Build 都能过
  - 证据：`pnpm type-check` / `pnpm lint:check` / `pnpm test` / `pnpm build` 本地均 exit=0（见 `docs/plans/archive/2026-03-16-root-planning-files/notes.md` 的命令记录摘要）
...
- ✅ i18n 结构一致性有自动化校验
  - 证据：`pnpm validate:translations` 显示 en/zh key 都是 847（见 `docs/plans/archive/2026-03-16-root-planning-files/notes.md`）
...
- 证据：`pnpm unused:production` 报告 `Unlisted binaries (1) next`（见 `docs/plans/archive/2026-03-16-root-planning-files/notes.md` 的命令摘要）。这和 “next 不在 production deps” 的现状一致。
...
- 证据：`pnpm test` / `pnpm test:coverage` 过程中出现 stderr 噪音（见 `docs/plans/archive/2026-03-16-root-planning-files/notes.md` 中记录的示例：Turnstile/Resend error 测试路径输出堆栈与错误日志）
...
- `pnpm unused:check` 报告 2 个 Unused devDependencies（更像是“knip 不懂配置文件消费”，不是实际未用）
  - 证据：`docs/plans/archive/2026-03-16-root-planning-files/notes.md` 中 `pnpm unused:check` 输出摘要；配置见 `knip.jsonc#L43`~`#L59`（已有 ignoreDependencies，但没覆盖 prettier plugins）
- `pnpm unused:production` 的输出与 `unused:check` 明显矛盾，需要你决定到底信哪个
  - 证据：`docs/plans/archive/2026-03-16-root-planning-files/notes.md` 的两个命令摘要（一个只报 2 个 devDeps，一个报 28 个 deps + next binary）
- `pnpm circular:check` 显示 “358 warnings” 但没给明细，工具输出不可执行
  - 证据：`docs/plans/archive/2026-03-16-root-planning-files/notes.md` 命令摘要（madge 输出只有计数，没有具体告警列表）
...
- knip 输出已收敛为“可执行信号”：
  ...
  - 证据：`docs/plans/archive/2026-03-16-root-planning-files/notes.md` 的 “P3 整改落地” 段落；配置：`knip.jsonc`、`tsconfig.knip.json`、`package.json` scripts
- madge warnings 从 358 降到 2：
  - `pnpm circular:check` 通过 `--ts-config tsconfig.json` 对齐 TS path alias/解析规则
  - 证据：`docs/plans/archive/2026-03-16-root-planning-files/notes.md` 的 “P3 整改落地” 段落；脚本：`package.json`
