...
const {
  ...
  mockGetProductStandardsCached: vi.fn(),
  mockSuspenseState: {
    ...
    Suspense: () => {
      ...
      return (
        <main className="container mx-auto px-4 py-8 md:py-12">
          ...
          {categories.length > 0 && (
            <div data-testid="category-filter" data-all-label="All Categories">
              {categories.map((cat) => (
                <span key={cat} data-testid={`category-${cat}`}>
                  {cat}
                </span>
              ))}
            </div>
          ...
          >
            {products.map((product) => (
              <div key={product.slug} data-testid={`product-${product.slug}`}>
                {product.title}
              </div>
            ))}
          </div>
...
vi.mock("@/lib/content/products", () => ({
  ...
  getProductStandardsCached: mockGetProductStandardsCached,
}));
...
vi.mock("@/app/[locale]/products/product-category-filter", () => ({
  ProductCategoryFilter: ({
    ...
    <div data-testid="category-filter" data-all-label={allCategoriesLabel}>
      {categories.map((cat) => (
        <span key={cat} data-testid={`category-${cat}`}>
          {cat}
        </span>
...
describe("ProductsPage", () => {
  const mockTranslations = {
    ...
    standardsFilterAriaLabel: "Filter by standards",
  ...
  beforeEach(() => {
    ...
    mockGetProductStandardsCached.mockResolvedValue(["ul651_sch80", "astm"]);
