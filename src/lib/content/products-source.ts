...
function assignArrayFields(detail: ProductDetail, metadata: ProductMetadata): void {
  ...
  if (metadata.standards !== undefined) detail.standards = metadata.standards;
...
export function getProductStandards(locale: Locale): ProductStandardId[] {
  ...
  for (const product of products) {
    const productStandards = product.metadata.standards;
    if (productStandards === undefined) continue;

    for (const standard of productStandards) {
